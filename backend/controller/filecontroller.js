...
const checkProjectAcess = async (projectId, userId, requiredRole = 'viewer') => {
    ...
    if (project.owener.toString() === userId.toString()) {
...
const getLangugaeExtension = (fileName) => {
...
const updateFile = asyncHandler(async (req, res) => {
    ...
    if (language && language !== file.language) {
        ...
        file.language = getLangugaeExtension(file.name);
    }

    if (name && !file.language) {
        file.language = getLangugaeExtension(name);
    }
...
const deleteComment = asyncHandler(async (req, res) => {
    ...
    const commet = file.comments.id(commentId);
    if (!commet) {
        res.status(404);
